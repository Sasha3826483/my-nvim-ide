...
docker rmi neovim 1>/dev/null 2>>log && echo "OK: Образ успешно удален" \
    || { echo "ERROR: Ошибка удаления контейнера"; exit 1; }

sleep 3

echo "Сборка образа..."
docker build -t neovim . && echo "OK: Образ успешно создан" \
    || { echo "ERROR: Ошибка сборки образа"; exit 1; }

sleep 3

echo "Запуск контейнера..."
docker run -it -d --name neovim \
